...
(() => {
  ...
  const stylesheet = `
    :host {
      ...
      /* Brand hooks — the thumbnail rail stays a neutral dark chrome, but its
         accent (current-slide outline + drop indicator) follows the deck's
         brand. Override per-deck from the page CSS, e.g.
         deck-stage { --deck-rail-accent: #1D6D40 } */
      --deck-rail-bg: #141414;
      --deck-rail-accent: #D97757;
    }
    ...
    .rail {
      ...
      background: var(--deck-rail-bg, #141414);
    ...
    .thumb[data-current] .frame { outline-color: var(--deck-rail-accent, #D97757); }
    ...
    .thumb::before {
      ...
      height: 3px;
      border-radius: 2px;
      background: var(--deck-rail-accent, #D97757);
      opacity: 0;
      pointer-events: none;
    }
